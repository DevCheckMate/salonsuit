...
def clear_all_tables():
    with Session(engine) as session:
        ...
        for table in metadata.sorted_tables:
            session.execute(text(f"DELETE FROM {table.name};"))  # Limpa os dados
            
            # Reseta o autoincrement apenas se sqlite_sequence existir
            if "sqlite_sequence" in session.execute(
                text("SELECT name FROM sqlite_master WHERE type='table' AND name='sqlite_sequence';")
            ).scalars().all():
                session.execute(text(f"DELETE FROM sqlite_sequence WHERE name='{table.name}';"))
